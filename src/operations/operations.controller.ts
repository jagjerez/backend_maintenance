...
export class OperationsController {
  ...
  @ApiResponse({ status: 201, description: 'Operation created successfully', type: Operation })
  @ApiResponse({ status: 409, description: 'Operation with this internal code already exists' })
  ...
  @ApiQuery({ name: 'type', required: false, enum: ['number', 'text', 'date', 'time', 'datetime', 'boolean'], description: 'Filter by type' })
  @ApiQuery({ name: 'companyId', required: false, type: String, description: 'Company ID' })
  async findAll(@Query() query: OperationQueryDto): Promise<PaginationResult<Operation>> {
    return this.operationsService.findAll(query);
  }

  @Get('deleted')
  @ApiOperation({ summary: 'Get all deleted operations' })
  @ApiResponse({ status: 200, description: 'Deleted operations retrieved successfully' })
  @ApiQuery({ name: 'companyId', required: true, type: String, description: 'Company ID' })
  async findDeleted(@Query('companyId') companyId: string): Promise<Operation[]> {
  ...
  @ApiResponse({ status: 200, description: 'Operation retrieved successfully', type: Operation })
  ...
  @ApiParam({ name: 'id', description: 'Operation ID' })
  @ApiResponse({ status: 200, description: 'Operation updated successfully', type: Operation })
  @ApiResponse({ status: 404, description: 'Operation not found' })
  @ApiResponse({ status: 409, description: 'Operation with this internal code already exists' })
  @ApiQuery({ name: 'companyId', required: true, type: String, description: 'Company ID' })
  async update(
  ...
  @ApiParam({ name: 'id', description: 'Operation ID' })
  @ApiResponse({ status: 200, description: 'Operation restored successfully', type: Operation })
  @ApiResponse({ status: 404, description: 'Operation not found or not deleted' })
  @ApiQuery({ name: 'companyId', required: true, type: String, description: 'Company ID' })
  async restore(
    @Param('id') id: string,
    @Query('companyId') companyId: string,
